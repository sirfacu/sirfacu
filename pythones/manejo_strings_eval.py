...
for daytrans in daily_transactions:
  daily_transactions_split.append(daytrans.split("!!"))

transactions_clean=[]
for tx in daily_transactions_split:
  for index in range(len(tx)):
    transactions_clean.append(tx[index].strip())
#print(transactions_clean)

customers = []
sales = []
thread_sold = []
lenght=len(transactions_clean)-4
...
for color in thread_sold:
  validate=color.split("&")
  if len(validate)==1:
    thread_sold_split.append(validate[0])
  else:
    for colors in validate:
...
for color in colors:
  print("The color "+ color + " exists " + str(color_count(color)) + " units.")